...
-- Execute the below queries before solving the next question. (If you haven't created the table 'manu' already)
create table manu (
...
with ship_table as
(
select Ship_Mode, month(Ship_Date) as Shipment_Month, count(*) as 'Number_Of_Orders'
from shipping_dimen
group by month(Ship_Date), Ship_Mode
order by Ship_Mode, month(Ship_Date) 
)
select *,
rank() over(partition by Ship_Mode order by Number_Of_Orders) 'Rank',
dense_rank() over(partition by Ship_Mode order by Number_Of_Orders) 'Dense Rank'
from ship_table;
...
avg(Shipping_Cost) over(order by year(Ship_Date), month(Ship_Date) rows unbounded preceding) 'Moving_Average',
avg(Shipping_Cost) over(order by year(Ship_Date), month(Ship_Date) rows 9 preceding) 'Prev 10 Moving Average'
from market_fact_full
...
-- -----------------------------------------------------------------------------------------------------------------
-- Session: Programming Constructs, Stored Functions and Procedures
-- IF Statements

-- 1. Classify an order as 'Profitable' or 'Not Profitable'.
select Market_fact_id, Ord_id, Profit,
if(Profit > 0, 'Profitable', 'Not Profitable') as Order_Info
from market_fact_full;
...
(
	...
	select *,
	percent_rank() over(order by Total_Sales desc) as 'Customer_Rank'
	from raw_data
)
select *,
	(case
		when Customer_Rank < 0.2 then 'GOLD'
        when Customer_Rank between 0.2 and 0.55 then 'SILVER'
        else 'BRONZE'
...
create procedure ClassifyMarketFact(
    in  m_Market_fact_id int, 
    out Market_Fact_Report varchar(50)
)
begin
    declare Profit int;

    case
        when Profit < -500 then
           set Market_Fact_Report = 'Huge loss';
        when Profit between -500 and 0 then
           set Market_Fact_Report = 'Bearable loss';
		when Profit between 0 and 500 then
           set Market_Fact_Report = 'Decent profit';
        else
           set Market_Fact_Report = 'Great profit';
    end case;
...
call ClassifyMarketFact(1234, @Market_Fact_Report);
select @Market_Fact_Report;

call ClassifyMarketFact(5678, @Market_Fact_Report);
select @Market_Fact_Report;

call ClassifyMarketFact(90, @Market_Fact_Report);
select @Market_Fact_Report;